...
@app.post("/weight")
def create_weight(weight:float,date:date,item : UserCreateWeight, db : Session = Depends(get_db)):
    message = create_weight_in_db(weight = weight,date=date,data = item, db = db)
    return message
